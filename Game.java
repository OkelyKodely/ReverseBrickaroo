...
public class Game implements KeyListener {
    ...
    public void initClouds() {
        ...
        for(int x=0; x<f; x++) {
            ...
            cloud.y = 100 + random.nextInt(580);
            clouds.add(cloud);
        }
    }
    ...
    public void initBricks() {
        ...
        for(int x=0; x<f; x++) {
            ...
            brick.y = 100 + random.nextInt(600);
            bricks.add(brick);
        }
        if(continits >= 10)
        for(int x=0; x<15; x++) {
    ...
    public void update() {
        ...
        for(int i=0; i<bricks.size(); i++) {
            ...
            {
                if(started) {
                    bird.x = bricks.get(i).x;
                    bird.y = bricks.get(i).y + bird.height;
                    started = false;
                }
            }
            g = false;
            if(bricks.get(i).y < 300)
                g = true;
            else
                count++;
            try {
                if(bricks.get(i).y < -60) {
        ...
        if(koreanWord.y < 0 || koreanWord.theSample.length() == 0){
